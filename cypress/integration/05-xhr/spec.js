...
it('starts with zero items (stubbed response)', () => {
  ...
  // THEN visit the page
  cy.server();
  cy.route('GET', '/todos', []).as('todos'); // This seems not working, does not set todos to empty array
  cy.visit('/');
  cy.wait('@todos') // wait for `GET /todos` response
    // inspect the server's response
    .its('response.body')
...
it('handles 404 when loading todos', () => {
  ...
  cy.get('@console-error').should('have.been.calledWithExactly', 'test does not allow it');
});
